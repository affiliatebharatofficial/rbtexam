...
import { Sparkles, Clock, ArrowRight, CheckCircle2, ShieldAlert, Unlock, Zap } from 'lucide-react';
...
export function TrialBanner() {
  ...
  // If Free Access Mode is active, show encouraging 100% unlocked banner
  if (isFree) {
    return (
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-950 via-slate-900 to-teal-950 p-5 text-white shadow-xl border border-emerald-500/30 mb-6">
        <div className="absolute top-0 right-0 -mr-16 -mt-16 w-64 h-64 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none" />
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative z-10">
          <div className="flex items-start gap-3.5">
            <div className="p-2.5 rounded-xl bg-emerald-500/20 border border-emerald-400/30 text-emerald-300 shrink-0 mt-0.5 sm:mt-0">
              <Unlock className="w-5 h-5 text-emerald-400" />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 flex items-center gap-1">
                  <CheckCircle2 className="w-3 h-3" /> 100% Free Complete Access Active
                </span>
                <span className="text-xs text-emerald-200/80 font-medium">All Questions & Tools Unlocked</span>
              </div>
              <h3 className="text-base font-bold text-white mt-1">
                You have full access to all BACB Mock Exams, Answers, Flashcards & Socrates AI Tutor!
              </h3>
              <p className="text-xs text-emerald-200/80 mt-0.5">
                Practice unrestricted with all 85-question exams and clinical rationales. No payment required.
              </p>
            </div>
          </div>
          ...
          <div className="shrink-0 w-full sm:w-auto flex items-center gap-3">
            <Link
              href="/exam"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs transition-all shadow-md hover:shadow-emerald-500/25 active:scale-95"
            >
              <Zap className="w-3.5 h-3.5 fill-white" />
              <span>Start 85-Question Mock Exam</span>
